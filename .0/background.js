...
chrome.contextMenus.onClicked.addListener(async function (info, tab) {
  if (info.menuItemId === 'resumeTexte') {
    ...
    try {
      ...
      // Utilisez chrome.scripting.executeScript pour afficher le résumé dans une alerte
      chrome.scripting.executeScript({
        target: { tabId: tab.id },
        func: function (summaryText) {
          alert(`Résumé:\n${summaryText}`);
        },
        args: [summary],
      });
      ...
      // Utilisez chrome.scripting.executeScript pour afficher l'erreur dans une alerte
      chrome.scripting.executeScript({
        target: { tabId: tab.id },
        func: function () {
